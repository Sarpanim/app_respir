...
import { NavItem, GeneralSettings } from '../../types';
import { AVAILABLE_ICONS, PRO_ICONS, DEFAULT_HEADER_NAV_ITEMS } from '../../constants';
import { DevicePhoneMobileIcon, XMarkIcon, ArrowUpTrayIcon, ImageIcon, ChevronDownIcon, ComputerDesktopIcon, Bars3Icon, ArrowPathIcon, CheckIcon, TrashIcon } from '../Icons';
import DynamicIcon from '../DynamicIcon';
import UrlSelector from './UrlSelector';

const AdminNavManagement: React.FC = () => {
    const { generalSettings, updateGeneralSettings } = useAppContext();
    const [headerNavItems, setHeaderNavItems] = useState<NavItem[]>(generalSettings.headerNavItems);
    const [isIconPickerOpen, setIsIconPickerOpen] = useState<number | null>(null);
    const [hasChanges, setHasChanges] = useState(false);

    useEffect(() => {
        setHeaderNavItems(generalSettings.headerNavItems);
    }, [generalSettings.headerNavItems]);
    ...
    const handleAddItem = () => {
        setHeaderNavItems([...headerNavItems, { id: `nav-${Date.now()}`, label: 'Nouveau lien', link: '/', icon: 'SparklesIcon' }]);
        setHasChanges(true);
    };
    ...
    const handleSave = () => {
        updateGeneralSettings({ ...generalSettings, headerNavItems });
        setHasChanges(false);
    };
    
    const handleReset = () => {
        if (window.confirm("Réinitialiser la navigation par défaut ?")) {
            setHeaderNavItems(DEFAULT_HEADER_NAV_ITEMS);
            setHasChanges(true);
        }
    };
    ...
    return (
        <AdminPageLayout title={
            <div className="flex items-center gap-3">
                ...
                <span>Gestion de la Navigation Principale</span>
            </div>
        }>
            <div className="flex justify-end gap-2 mb-4">
                <button onClick={handleReset} className="px-4 py-2 text-sm font-semibold text-white bg-gray-500 rounded-lg hover:bg-gray-600">
            ...
            <div className="bg-white/30 dark:bg-black/20 p-4 rounded-xl border border-white/20 dark:border-black/30 space-y-3">
                {headerNavItems.map((item, index) => (
                    <div key={item.id} className="flex flex-col md:flex-row items-center gap-3 p-3 bg-black/5 dark:bg-white/5 rounded-lg">
