...
const CourseEditModal: React.FC<CourseEditModalProps> = ({ course, onClose, onSave }) => {
    ...
    const handleSaveSection = (section: Section) => {
        if (section.id && editedCourse.sections.some(s => s.id === section.id)) {
            ...
            const newLessons = section.lessons ? section.lessons : [];
            setEditedCourse(prev => ({
                ...prev,
                sections: [...prev.sections, { ...section, id: uuidv4(), lessons: newLessons }]
            }));
        }
    ...
    // Lesson handlers
    const handleSaveLesson = (lesson: Lesson, sectionId: string) => {
        const newSections = editedCourse.sections.map(section => {
            if (section.id === sectionId) {
                if (lesson.id && section.lessons.some(l => l.id === lesson.id)) {
                    return { ...section, lessons: section.lessons.map(l => l.id === lesson.id ? lesson : l) };
                } else {
                    return { ...section, lessons: [...section.lessons, { ...lesson, id: uuidv4() }] };
                }
            }
            return section;
        });
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[101] p-4 animate-fade-in" onClick={onClose}>
            <div className="bg-[#2C2C2E] text-dark-text rounded-2xl shadow-2xl w-full max-w-3xl m-4 relative border border-white/10 flex flex-col h-[90vh]" onClick={e => e.stopPropagation()}>
                <div className="flex-shrink-0 p-6 flex justify-between items-center border-b border-white/10">
                    <h3 className="text-2xl font-elsie font-bold">Éditer le Cours</h3>
                    ...
                    {activeTab === 'general' && (
                        <div className="space-y-6">
                            <FormField label="Titre du cours">
                                <input name="title" value={editedCourse.title} onChange={handleChange} className={inputClasses} />
                            </FormField>
                            <FormField label="Description">
                                <textarea name="description" value={editedCourse.description} onChange={handleChange} rows={5} className={inputClasses}></textarea>
                            </FormField>
                            ...
                            <FormField label="Image de couverture">
                                <div className="flex items-center gap-4">
                                    <img src={editedCourse.image.url || 'https://i.pravatar.cc/150?u=cat'} alt="Aperçu" className="w-20 h-20 object-cover rounded-lg bg-black/20" />
                                    <div>
                             ...
                             <FormField label="Tags">
                                <div className={`flex flex-wrap items-center gap-2 p-2 rounded-lg ${inputClasses}`}>
                                    ...
                                    <input type="text" value={currentTag} onChange={e => setCurrentTag(e.target.value)} onKeyDown={handleAddTag} placeholder="Ajouter un tag..." className="bg-transparent outline-none flex-grow" />
                                </div>
                            </FormField>
                        </div>
                    )}
                     {activeTab === 'structure' && (
                        <div>
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button onClick={() => setEditingSection({ title: '' })} className="flex items-center gap-2 px-4 py-2 bg-accent text-white rounded-lg font-semibold text-sm hover:bg-accent/90 transition-colors">
